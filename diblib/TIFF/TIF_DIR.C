...
static  MissingRequired(PTIFF , char *);
static  TIFFFetchData(PTIFF , TIFFDirEntry FAR *, char FAR *);
static  TIFFFetchString(PTIFF , TIFFDirEntry FAR *, char FAR *);
static  double TIFFFetchRational(PTIFF , TIFFDirEntry FAR *);
static  TIFFUnpackShorts(PTIFF , u_short FAR *, TIFFDirEntry FAR *);
static  TIFFFetchPerSampleShorts(PTIFF , TIFFDirEntry FAR *, long FAR *);
static  TIFFFetchStripThing(PTIFF , TIFFDirEntry FAR *, long, u_long FAR* FAR*);
static  TIFFFieldInfo *FindFieldInfo(u_short, TIFFDataType);
static  TIFFFieldInfo *FieldWithTag(u_short);
static  TIFFGetField1(TIFFDirectory FAR *, int, va_list);
#else
...
#endif
#pragma alloc_text(TIFF2,TIFFReadDirectory,TIFFWriteDirectory,TIFFFetchStripThing)
        ...
        PTIFF tif;
{
        ...
        register int n;
        register TIFFDirectory FAR *td = NULL;
        TIFFDirEntry FAR *dir = NULL;
        long v;
        TIFFFieldInfo *fip;
        u_short dircount;
        char FAR *cp;
        ...
        dir = (TIFFDirEntry FAR *)malloc(dircount * sizeof (TIFFDirEntry));
        if (dir == NULL) {
        ...
        "Invalid TIFF directory; tags are not sorted in ascending order");
                ...
                if (fip >= &FieldInfo[NFIELDINFO] ||
                         fip->field_tag != dp->tdir_tag) {
                        TIFFWarning(tif->tif_name,
                                 "ignoring unknown field with tag %u (0x%x)",
        ...
        for (dp = dir, n = dircount; n > 0; n--, dp++) {
                ...
                if (dp->tdir_type == (short)TIFF_ASCII) {
                        if (dp->tdir_count > 0) {
                                ...
                                ok = (TIFFFetchString(tif, dp, cp) &&
                                         TIFFSetField(tif, dp->tdir_tag, (long)cp));
                                free(cp);
                                if (!ok)
                                        goto bad;
                        }
                ...
                case TIFFTAG_COLORMAP:
                        ...
                        ok = (TIFFFetchData(tif, dp, cp) &&
                                 TIFFSetField(tif, dp->tdir_tag, (long)cp,(long)cp+v,(long)cp+2*v));
                        free(cp);
                        if (!ok)
                                goto bad;
                        break;
...
"TIFF directory is missing required \"%s\" field, calculating from imagelength",
                ...
                td->td_stripbytecount = (u_long FAR *)malloc(sizeof (u_long));
        ...
        u_long FAR * FAR * lpp;
{
         ...
         */
        if (*lpp == NULL &&
                 (*lpp = (u_long FAR *)malloc(nstrips * sizeof (u_long))) == NULL) {
        ...
        if (dir->tdir_type == (int)TIFF_SHORT) {
                ...
                if (dir->tdir_count > 2) {
                        ...
                        if (status = TIFFFetchData(tif, dir, dp)) {
                                register u_short FAR *wp = (u_short FAR *)dp;
                                while (nstrips-- > 0)
                                        *lp++ = *wp++;
                        }
                        free(dp);
                ...
                if (dir->tdir_count > 1)
                        status = TIFFFetchData(tif, dir, (char FAR *)lp);
                else
                        *lp = dir->tdir_offset;
        }
        return (status);
        ...
        PTIFF tif;
{
        ...
        if  (td->td_stripoffset) {
                free((char FAR *)td->td_stripoffset);
                td->td_stripoffset = NULL;
        }
        if  (td->td_stripbytecount) {
                free((char FAR *)td->td_stripbytecount);
...
static
setCurve(wpp, wp, nw)
        u_short FAR * FAR *wpp, FAR *wp;
        long nw;
{
        if (*wpp)
                free((char FAR *)*wpp), *wpp = 0;
        nw *= sizeof (u_short);
        if (wp && (*wpp = (u_short FAR *)malloc(nw)))
                bcopy(wp, *wpp, nw);
}
 ...
 */
/*VARARGS2*/
DECLARE2V(TIFFSetField, PTIFF, tif, long, tag)
{
        ...
        case TIFFTAG_COLORRESPONSECURVE:
                ...
                setCurve(&td->td_redresponsecurve, va_arg(ap, u_short FAR *), v);
                setCurve(&td->td_greenresponsecurve, va_arg(ap, u_short FAR *), v);
                setCurve(&td->td_blueresponsecurve, va_arg(ap, u_short FAR *), v);
                field = FIELD_COLORRESPONSECURVE;
                break;
        case TIFFTAG_COLORMAP:
                v = 1L<<td->td_bitspersample;
                setCurve(&td->td_redcolormap, va_arg(ap, u_short FAR *), v);
                setCurve(&td->td_greencolormap, va_arg(ap, u_short FAR *), v);
                setCurve(&td->td_bluecolormap, va_arg(ap, u_short FAR *), v);
 ...
 */
/*VARARGS2*/
DECLARE2V(TIFFGetField, PTIFF, tif, int, tag)
{
        TIFFFieldInfo *fip = FindFieldInfo(tag, (TIFFDataType)0);
        ...
        va_list ap;
{
        ...
        case TIFFTAG_FILLORDER:
                *va_arg(ap, u_short *) = td->td_fillorder;
                break;
        case TIFFTAG_DOCUMENTNAME:
                *va_arg(ap, char FAR **) = td->td_documentname;
                break;
        case TIFFTAG_ARTIST:
                *va_arg(ap, char FAR **) = td->td_artist;
                break;
        case TIFFTAG_DATETIME:
                *va_arg(ap, char FAR **) = td->td_datetime;
                break;
        case TIFFTAG_HOSTCOMPUTER:
                *va_arg(ap, char FAR **) = td->td_hostcomputer;
                break;
        case TIFFTAG_IMAGEDESCRIPTION:
                *va_arg(ap, char FAR **) = td->td_imagedescription;
                break;
        case TIFFTAG_MAKE:
                *va_arg(ap, char FAR **) = td->td_make;
                break;
        case TIFFTAG_MODEL:
                *va_arg(ap, char FAR **) = td->td_model;
                break;
        case TIFFTAG_SOFTWARE:
                *va_arg(ap, char FAR **) = td->td_software;
                break;
        case TIFFTAG_ORIENTATION:
        ...
        case TIFFTAG_YPOSITION:
                *va_arg(ap, float *) = td->td_yposition;
                break;
        case TIFFTAG_PAGENAME:
                *va_arg(ap, char FAR **) = td->td_pagename;
                break;
        case TIFFTAG_GRAYRESPONSEUNIT:
                *va_arg(ap, u_short *) = td->td_grayresponseunit;
                break;
        case TIFFTAG_GRAYRESPONSECURVE:
                *va_arg(ap, u_short FAR **) = td->td_grayresponsecurve;
                break;
        case TIFFTAG_GROUP3OPTIONS:
        ...
        case TIFFTAG_COLORRESPONSEUNIT:
                *va_arg(ap, u_short *) = td->td_colorresponseunit;
                break;
        case TIFFTAG_COLORRESPONSECURVE:
                *va_arg(ap, u_short FAR **) = td->td_redresponsecurve;
                *va_arg(ap, u_short FAR **) = td->td_greenresponsecurve;
                *va_arg(ap, u_short FAR **) = td->td_blueresponsecurve;
                break;
        case TIFFTAG_COLORMAP:
                *va_arg(ap, u_short FAR **) = td->td_redcolormap;
                *va_arg(ap, u_short FAR **) = td->td_greencolormap;
                *va_arg(ap, u_short FAR **) = td->td_bluecolormap;
                break;
        case TIFFTAG_PREDICTOR:
                *va_arg(ap, u_short *) = td->td_predictor;
                break;
        case TIFFTAG_STRIPOFFSETS:
                *va_arg(ap, u_long FAR **) = td->td_stripoffset;
                break;
        case TIFFTAG_STRIPBYTECOUNTS:
                *va_arg(ap, u_long FAR **) = td->td_stripbytecount;
                break;
#ifndef MSDOS
        case TIFFTAG_MATTEING:
...
static
/*VARARGS2*/
DECLARE2V(TIFFgetfield, TIFFDirectory FAR *, td, int, tag)
{
        va_list ap;
...
#if USE_PROTOTYPES
static  TIFFWriteRational(PTIFF , u_short, TIFFDirEntry FAR *, float);
static  TIFFWritePerSampleShorts(PTIFF , u_short, TIFFDirEntry FAR *, short);
static  TIFFWriteStripThing(PTIFF , u_short, TIFFDirEntry FAR *, u_long FAR *);
static  TIFFWriteData(PTIFF , TIFFDirEntry FAR *, char FAR *);
static  TIFFLinkDirectory(PTIFF );
#else
...
#undef WRITE
#define WRITE(x) TIFFWriteData(tif, dir, (char FAR *) x)
...
DECLARE4(TIFFWriteRational,
         PTIFF, tif, u_short, tag, TIFFDirEntry FAR *, dir, float, v)
{
        long t[2];
...
DECLARE4(TIFFWritePerSampleShorts,
         PTIFF, tif, u_short, tag, TIFFDirEntry FAR *, dir, short, v)
{
        short w[4];
...
DECLARE4(TIFFWriteStripThing,
         PTIFF, tif, u_short, tag, TIFFDirEntry FAR *, dir, u_long FAR *, lp)
{
        ...
        if (dir->tdir_count > 1)
                return (TIFFWriteData(tif, dir, (char FAR *)lp));
        dir->tdir_offset = *lp;
        return (1);
}
        ...
        PTIFF tif;
{
        ...
        do {
                ...
                if (tif->tif_flags & TIFF_SWAB)
                        TIFFSwabLong((unsigned long _far *)&nextdir);
        } while (nextdir != 0);
