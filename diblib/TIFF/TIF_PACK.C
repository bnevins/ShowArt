...
#endif
#if USE_PROTOTYPES
static  int PackBitsEncode(PTIFF , u_char *, int);
static  int PackBitsDecode(PTIFF , u_char *, int);
#else
static  int PackBitsEncode(), PackBitsDecode();
#endif
#pragma alloc_text(TIFF1,TIFFInitPackBits,PackBitsEncode,PackBitsDecode)
...
PackBitsEncode(tif, bp, cc)
        ...
        register int cc;
{
        register char FAR *op, FAR *lastliteral;
        register int n, b;
